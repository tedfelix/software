...
2 ' File date: 8/3/1983 09:15:32 PM
5 DEFINT W,B,T,C,S,X,Y,P,M
8 STRIG ON:GOSUB 9000
9 ' Initialize
...
1000 SP(0)=S(0):SP(1)=S(1):SP(2)=S(2):SP(3)=S(3):CP=C:GOSUB 2000
1010 GOSUB 4000
1020 IF STRIG(5)THEN GOSUB 4000:M=M+1
1030 LINE(CP+W4,Y4)-(SP(3)+W3,Y3),0
1040 LINE(C+W4,Y4)-(S(3)+W3,Y3),CO
1050 LINE(CP-W4,Y4)-(SP(3)-W3,Y3),0
1060 LINE(C-W4,Y4)-(S(3)-W3,Y3),CO
1070 LINE(SP(3)+W3,Y3)-(SP(2)+W2,Y2),0
1080 LINE(S(3)+W3,Y3)-(S(2)+W2,Y2),C1
1090 LINE(SP(3)-W3,Y3)-(SP(2)-W2,Y2),0
1100 LINE(S(3)-W3,Y3)-(S(2)-W2,Y2),C1
1110 LINE(SP(2)+W2,Y2)-(SP(1)+W1,Y1),0
1120 LINE(S(2)+W2,Y2)-(S(1)+W1,Y1),C2
1130 LINE(SP(2)-W2,Y2)-(SP(1)-W1,Y1),0
1140 LINE(S(2)-W2,Y2)-(S(1)-W1,Y1),C2
1150 LINE(SP(1)+W1,Y1)-(SP(0)+W0,Y0),0
1160 LINE(S(1)+W1,Y1)-(S(0)+W0,Y0),C3
1170 LINE(SP(1)-W1,Y1)-(SP(0)-W0,Y0),0
1180 LINE(S(1)-W1,Y1)-(S(0)-W0,Y0),C3
1190 C3=C2:C2=C1:C1=CO:GOTO 1000
1990 'Car move
2000 X=STICK(0)-MX
2010 LOCATE 4,30:PRINT USING"##.##";M/100
2020 M=M+1:PA=XA:XM=X/(MX/25):XA=X/(MX/10)
2030 IF M=100 OR M=101 THEN CO=3:CHAM=100
2040 IF M=300 OR M=301 THEN CO=2:CHAM=130
...
3070 GOTO 30
3999 ' Push stack and make curve
4000 S(0)=S(1):S(1)=S(2):S(2)=S(3):S(3)=C
4010 C=C+FNRAND(CHAM-1)-CHAM/2
4020 RETURN
7000 INPUT"Would you like a random game";A$
7010 A$=LEFT$(A$,1):IF A$="Y" OR A$="y" THEN 7100 ELSE RETURN
7100 RANDOMIZE TIMER
7110 RETURN
