1 ' Gold Mine v2.00
2 ' Copyright 2017, Ted Felix.  File date: 12/31/1986 02:57:18 PM
...
230 IF RIGHT$(A$,1)="M"THEN E=0:X=35:GOTO 2000
240 GOTO 220
...
1007 IF LEN(A$)=2 THEN R$=RIGHT$(A$,1) ELSE R$=""
1009 ' Check input
1010 IF R$="K"THEN X=X-1:FL=1:P$=R$
1020 IF R$="M"THEN X=X+1:FL=1:P$=R$
1030 IF R$="H"THEN Y=Y-1:FL=1:P$=R$
1040 IF R$="P"THEN Y=Y+1:FL=1:P$=R$
...
2012 IF LEN(A$)=2 THEN R$=RIGHT$(A$,1) ELSE R$=""
2015 IF START=1 THEN A=1 ELSE A=2
2020 IF R$="P"THEN 2200
2030 IF R$="H"THEN 2300
2040 IF R$="K"THEN X=35:START=0:ON A GOTO 3000,1000
2070 GOTO 2010
2199 ' Elevator down
...
3145 IF LEN(A$)=2 THEN R$=RIGHT$(A$,1) ELSE R$=""
3150 IF R$="M"THEN 3210
3160 IF R$="K"THEN GOSUB 10000 ELSE 3140
3210 FOR I=25 TO 34
...
5999 ' Pump
6000 Y=PY:X=PX:IF P$="H"THEN Y=Y-1:IF Y<4 THEN Y=4:GOTO 1000
6010 IF P$="P"THEN Y=Y+1:IF Y>20 THEN Y=20:GOTO 1000
6020 IF P$="K"THEN X=X-1:IF X<2 THEN X=2:GOTO 1000
6030 IF P$="M"THEN X=X+1:IF X>33 THEN X=33:LOCATE PY,PX:PRINT" ";:GOTO 1000
...
6099 ' Drill
6100 Y=PY:X=PX:IF P$="H"THEN Y=Y-1:IF Y<4 THEN Y=4:GOTO 1000
6110 IF P$="P"THEN Y=Y+1:IF Y>20 THEN Y=20:GOTO 1000
6120 IF P$="K"THEN X=X-1:IF X<2 THEN X=2:GOTO 1000
6130 IF P$="M"THEN X=X+1:IF X>33 THEN 1000
